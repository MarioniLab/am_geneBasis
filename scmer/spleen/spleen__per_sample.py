...
data = pd.read_csv(root_dir + '4scmer/spleen/spleen.csv', header=0)
data = data.T
...
for current_sample in samples:
    ...
    current_obs = current_data[['sample', 'celltype']]
    current_data.drop(['sample', 'celltype'], axis=1, inplace=True)
    adata = sc.AnnData(current_data)
    for i in current_obs.columns:
        adata.obs[i] = current_obs[i]
