---
title: "Pipeline to assess how different Lp-norms affect selections (mouse embryo)."
output:
...
get_umaps = function(meta, genes, titles, wide = T){
  plots = lapply(1:length(genes) , function(i){
    gene = genes[i]
    if (!is.null(titles)){
      title = paste(rowdata$gene_symbol[rowdata$gene == gene], ", gene-score = ", titles[i])
    } else {
      title = rowdata$gene_symbol[rowdata$gene == gene]
    }
    counts = data.frame(cell = colnames(sce) , counts = as.numeric(assay(sce , "logcounts")[gene , ]))
