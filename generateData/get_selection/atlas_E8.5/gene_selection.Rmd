---
title: "Gene selection."
output:
  BiocStyle::html_document:
    code_folding: hide
    number_sections: yes
    toc: yes  
---

#Introduction & method


```{r load, message = FALSE}
# load libraries we are going to use

library(Matrix)
library(scran)
library(scater)
library(batchelor)
library(knitr)
library(reshape2)
library(edgeR)
library(ggplot2)
library(BiocSingular)
library(BiocParallel)
library(BiocNeighbors)
library(SingleCellExperiment)
library(dplyr)
library(BiocStyle)
library(stats)
ncores = 10
mcparam = MulticoreParam(workers = ncores)
register(mcparam)
set.seed(32)

#root.dir = "/Users/alsu/Develop/geneBasis/"
root.dir = "/nfs/research1/marioni/alsu/geneBasis/"
source(paste0(root.dir , "am_geneBasis/functions/main_functions.R"))

system = "atlas_E8.5"
sce = readRDS(paste0(root.dir , "data/scRNA_datasets/" , system , "/sce_" , system , ".Rds"))

# reduce search - discard obviously uninteresting genes
sce = retain_only_hvgs(sce, n = 10000, var.thresh = 0)
  
# discard mitochondrial
rownames.sce = rowData(sce)[,2]
idx = which(!grepl("mt-" , rownames.sce) & !grepl("MT-" , rownames.sce) & !grepl("Mt-" , rownames.sce))
sce = sce[idx,]


```

# Iterative gene search

```{r iterative-gene-search, message = FALSE}


n_genes_total = 250

genes_batched = gene_search(sce , genes_base = NULL, n_genes_total = 3, batch = "sample", n.neigh = 5, p = 3, K = 5, nPC = NULL, ncores = ncores)
saveRDS(genes_batched , paste0(root.dir , "data/gene_selection/" , system , "/" , "gene_selection_" , "batched", ".Rds"))

genes_unbatched = gene_search(sce , genes_base = NULL, n_genes_total = 3, batch = NULL, n.neigh = 5, p = 3, K = 5, nPC = NULL, ncores = ncores)
saveRDS(genes_unbatched , paste0(root.dir , "data/gene_selection/" , system , "/" , "gene_selection_" , "unbatched", ".Rds"))
 

```

#Session Info

```{r sessinf}
sessionInfo()
```
