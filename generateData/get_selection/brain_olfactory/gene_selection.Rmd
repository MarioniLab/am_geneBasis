...
sce = sce[, !sce$Class %in% c("Blood" , "Excluded")]
...
unq_sce_genes = table(rownames(sce))
unq_sce_genes = names(unq_sce_genes)[unq_sce_genes == 1]
seqfish_genes = rownames(seqfish_olfactory)
seqfish_genes = intersect(unq_sce_genes , seqfish_genes)
sce = sce[rownames(sce) %in% seqfish_genes, ]
...
n_genes_total = 200
genes = geneBasisR::gene_search(sce , genes_base = NULL, n_genes_total = n_genes_total, batch = "DonorID", n.neigh = 5, nPC.all = 50, nPC.selection = 100, verbose = T)
