    ...
    for current_min_disp in min_disp:
        for current_n_threads in n_threads:
            for current_lambda in lambdas:
                ...
                sc.pp.highly_variable_genes(current_adata, min_mean=0.01, max_mean=10, min_disp=current_min_disp)
                ...
                df.to_csv(save_dir + 'stat_batched' + t1 + '.csv', header=True, index=None, sep=',')   
...
df_all.to_csv(save_dir + 'stat_batched_combined.csv', header=True, index=None, sep=',')  
