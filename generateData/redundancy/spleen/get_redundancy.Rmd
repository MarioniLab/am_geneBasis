...
stat = bplapply(n_genes.grid , function(n_genes){
  current.genes = as.character(genes_alsu[1:n_genes])
  stat.all = get_celltypes_stat(sce , genes = current.genes, batch = "sample", n.neigh = 5)
  colnames(stat.all) = c("celltype" , "frac_correctly_mapped_all")  
  stat.per_gene = lapply(current.genes , function(gene){
    print(gene)
    current.stat = get_celltypes_stat(sce , genes = setdiff(current.genes,gene) , batch = "sample", n.neigh = 5)
