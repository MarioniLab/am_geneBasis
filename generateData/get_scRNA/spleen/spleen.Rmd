...
logcounts = data.matrix(logcounts , rownames.force = NA)
logcounts <- as(logcounts, "dgTMatrix") 
...
genes = intersect(rownames(logcounts) , rownames(counts))
cells = intersect(colnames(logcounts), colnames(counts))
counts = counts[rownames(counts) %in% genes, colnames(counts) %in% cells]
logcounts = logcounts[rownames(logcounts) %in% genes, colnames(logcounts) %in% cells]
...
saveRDS(counts, file = paste0(root.dir, "data/spleen/counts_spleen.Rds"))
saveRDS(logcounts, file = paste0(root.dir, "data/spleen/logcounts_spleen.Rds"))
saveRDS(sce, file = paste0(root.dir, "data/spleen/sce_spleen.Rds"))
