...
stat_gene_score = suppressWarnings( get_corr_per_gene(sce, genes = hvgs_10000, batch = NULL , n.neigh = 5 , nPC = 50 , genes.predict = hvgs_10000 ) ) 
...
get_gene_score_stat = function(stat , nPC){
  gene_score_stat = bplapply(c(1:length(genes)) , function(i){
    ...
    stat.per_n_genes = lapply(1:length(current.genes) , function(j){
      current.stat = suppressWarnings( get_corr_per_gene(sce , genes = current.genes[[j]], batch = NULL , n.neigh = 5 , nPC = nPC , genes.predict = hvgs_10000 ) )
      if (is.null(nPC)){
