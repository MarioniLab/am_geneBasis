...
stat_gene_score = suppressWarnings( get_corr_per_gene(sce, genes = rownames(sce), batch = "sample" , n.neigh = 5 , nPC = 50 , genes.predict = rownames(sce)) ) 
...
stat_scmer_batched = bplapply(c(1:length(genes)) , function(i){
  ...
  stat.per_n_genes = lapply(c(1:length(current.genes)) , function(j){
    ...
    current.stat.nPC_50 = suppressWarnings( get_corr_per_gene(sce, genes = current.genes[[j]], batch = "sample" , n.neigh = 5 , nPC = 50 , genes.predict = rownames(sce)) ) 
    ...
    current.stat.nPC_all = suppressWarnings( get_corr_per_gene(sce, genes = current.genes[[j]], batch = "sample" , n.neigh = 5 , nPC = length(current.genes[[j]]) - 1 , genes.predict = rownames(sce)) ) 
