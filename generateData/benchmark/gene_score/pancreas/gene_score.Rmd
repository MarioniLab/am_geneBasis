...
stat_gene_score = suppressWarnings( get_distr_dist(sce, genes = hvgs_5000, assay = "logcounts" , batch = "sample" , n.neigh = 5 , nPC = 50 , genes.predict = genes.predict , type = "corr") ) 
...
stat_scmer_batched = bplapply(c(1:length(genes)) , function(i){
  ...
  stat.per_n_genes = lapply(c(1:length(current.genes)) , function(j){
    current.stat = suppressWarnings( get_distr_dist(sce , genes = current.genes[[j]] , assay = "logcounts" , batch = "sample", n.neigh = 5 , nPC = 50 , genes.predict = genes.predict , type = "corr") )
