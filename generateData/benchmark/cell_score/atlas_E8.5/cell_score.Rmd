...
genes = lapply(1:length(genes_scmer_batched) , function(i){
  ...
  out = list("alsu" = genes_alsu[1:n_genes] , "scmer_batched" = current.genes_scmer , "hvgs" = get_hvgs(n_genes))
  return(out)
})

stat_scmer_batched = lapply(c(1:length(genes)) , function(i){
  current.genes = genes[[i]]
  stat.per_n_genes = bplapply(1:length(current.genes) , function(j){
...
genes = lapply(1:length(genes_scmer_unbatched) , function(i){
  ...
  n_genes = length(current.genes_scmer)
  out = list("alsu" = genes_alsu[1:n_genes] , "scmer_unbatched" = current.genes_scmer, "hvgs" = get_hvgs(n_genes))
  return(out)
})
