...
## SCMER - on wider search grid

```{r read-selection-scmer-updated, message = FALSE}


# batched
scmer_batched.files = list.files(path = paste0( root.dir , "data/scmer_res_selected/" , system , "_higher_mean/batched/" ))
n_genes = sapply(scmer_batched.files , function(str) str_replace_all(str,  ".txt" , ""))
n_genes = sapply(n_genes , function(str) str_replace_all(str,  "nGenes_" , ""))
n_genes = sort(as.numeric(n_genes))

genes_scmer_batched_updated = lapply(n_genes , function(str){
  out = read.table(paste0( root.dir , "data/scmer_res_selected/" , system , "_higher_mean/batched/nGenes_" , str , ".txt") , header = F)
  genes = out$V1
  return(genes)
})
...
genes_scmer_unbatched_updated = lapply(n_genes , function(str){
...
get_cell_score_stat = function(genes , nPC){
  cell_score_stat = bplapply(c(1:length(genes)) , function(i){
    ...
    stat.per_n_genes = lapply(1:length(current.genes) , function(j){
      current.stat = suppressWarnings( get_preservation_score(sce , neighs.all = neighs.all ,  genes.all = hvgs_10000, 
                                                 genes.compare = current.genes[[j]], batch = "sample" ,n.neigh = 5 , nPC.all = 50 , nPC.compare = nPC) )
      if (is.null(nPC)){
...
genes = lapply(1:length(genes_scmer_batched) , function(i){
  ...
  out = list("geneBasis" = as.character(genes_alsu[1:n_genes]) , "SCMER" = as.character(current.genes_scmer) , "hvgs" = as.character(get_hvgs(sce,n=n_genes)))
  return(out)
})

stat_scmer_batched.nPC_50 = get_cell_score_stat(genes , nPC = 50)
stat_scmer_batched.nPC_all = get_cell_score_stat(genes , nPC = NULL)
...
## Alsu VS scmer-batched - updated

```{r score-vs-scmer-batched-updated, message = FALSE}


genes = lapply(1:length(genes_scmer_batched_updated) , function(i){
  current.genes_scmer = genes_scmer_batched_updated[[i]]
  n_genes = length(current.genes_scmer)
  out = list("geneBasis" = as.character(genes_alsu[1:n_genes]) , "SCMER" = as.character(current.genes_scmer) , "hvgs" = as.character(get_hvgs(sce,n=n_genes)))
  return(out)
})
...
stat_scmer_batched = rbind(stat_scmer_batched.nPC_50 , stat_scmer_batched.nPC_all)
stat_scmer_batched$type = factor(stat_scmer_batched$type, levels = c("geneBasis" , "SCMER" , "hvgs"))
saveRDS(stat_scmer_batched , file = paste0(root.dir , "data/benchmark/cell_score/" , system , "/cell_score__scmer_batched_updated.Rds"))
...
stat_scmer_unbatched.nPC_50 = get_cell_score_stat(genes , nPC = 50)
stat_scmer_unbatched.nPC_all = get_cell_score_stat(genes , nPC = NULL)
...
## Alsu VS scmer-unbatched - updated

```{r score-vs-scmer-unbatched-updated, message = FALSE}


genes = lapply(1:length(genes_scmer_unbatched_updated) , function(i){
  current.genes_scmer = genes_scmer_unbatched_updated[[i]]
  n_genes = length(current.genes_scmer)
  out = list("geneBasis" = as.character(genes_alsu[1:n_genes]) , "SCMER" = as.character(current.genes_scmer) , "hvgs" = as.character(get_hvgs(sce,n=n_genes)))
  return(out)
})
...
stat_scmer_unbatched = rbind(stat_scmer_unbatched.nPC_50 , stat_scmer_unbatched.nPC_all)
stat_scmer_unbatched$type = factor(stat_scmer_unbatched$type, levels = c("geneBasis" , "SCMER" , "hvgs"))
saveRDS(stat_scmer_unbatched , file = paste0(root.dir , "data/benchmark/cell_score/" , system , "/cell_score__scmer_unbatched_updated.Rds"))
...
stat_scgenefit.nPC_50 = get_cell_score_stat(genes , nPC = 50)
stat_scgenefit.nPC_all = get_cell_score_stat(genes , nPC = NULL)
