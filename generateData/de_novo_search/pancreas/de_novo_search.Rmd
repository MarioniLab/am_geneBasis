...
saveRDS( corr_stat_all , file = paste0(root.dir , "data/scRNA_datasets/" , system , "/" , system , "__corr_stat_all.Rds"))
...
genes_stat = lapply(min.corr.thresh.grid , function(min.corr.thresh){
  ...
  for (idx.batch in c(1:length(unq.batches))){
    ...
    while( min.corr < min.corr.thresh ){
      corr_stat_genes = suppressWarnings( get_corr_transcriptome_per_gene(current.sce , assay = "logcounts" , genes = genes_all , batch = NULL, n.neigh = 5 , nPC = min( length(genes_all) - 1 , ncol(current.sce) - 1), genes.predict = current.genes_keep , method = "spearman"))
