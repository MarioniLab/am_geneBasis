...
# keep only suitable genes
#genes.4seq = get_suitable4seq_genes("spleen" , mean.thresh = 7, max.thresh = 350)
#sce = sce[rownames(sce) %in% genes.4seq , ]
# assign reference and query to samples
#tab = table(sce$sample , sce$celltype)
...
mappings.dir = paste0(root.dir , "data/mappings/spleen/")
...
getCorrTranscriptome = function(mapping){
  if (class(mapping) == "list"){
    stat.per_nGenes = lapply(c(1,9,19), function(current.mapping.id){
      ...
      stat = bplapply(rownames(sce) , function(gene){
        
        current.logcounts = data.frame(cell = colnames(logcounts) , counts = logcounts[rownames(logcounts) == gene , ])
        
        current.stat = lapply(1:nrow(current.mapping) , function(i){
          idx.neighbors = which( current.logcounts$cell %in% current.mapping[i , cols.cells])
          idx.cell = which(current.logcounts$cell %in% current.mapping$cell[i])
          out = data.frame(cell = current.mapping$cell[i] , 
                           real.counts = current.logcounts$counts[idx.cell] ,
                           estimated.counts = mean( current.logcounts$counts[idx.neighbors] ) )
          return(out)
        })  
        current.stat = do.call(rbind , current.stat)
        out = data.frame(gene = gene , corr = cor(current.stat$estimated.counts , current.stat$real.counts , method = "pearson"))
        return(out)
      }, BPPARAM = mcparam)
...
saveRDS(stat.corr.weighted_unscaled__twoRounds , file = paste0(root.dir, "data/corr_stat/spleen/weighted_unscaled__twoRounds.Rds"))

stat.corr.unweighted_unscaled__twoRounds = getCorrTranscriptome(mappings.unweighted_unscaled__twoRounds)
saveRDS(stat.corr.unweighted_unscaled__twoRounds , file = paste0(root.dir, "data/corr_stat/spleen/unweighted_unscaled__twoRounds.Rds"))

stat.corr.bianca.pairwise_centers = getCorrTranscriptome(mappings.bianca.pairwise_centers)
saveRDS(stat.corr.bianca.pairwise_centers , file = paste0(root.dir, "data/corr_stat/spleen/bianca.pairwise_centers.Rds"))

stat.corr.all = getCorrTranscriptome(mappings.all)
saveRDS(stat.corr.all , file = paste0(root.dir, "data/corr_stat/spleen/all.Rds"))
