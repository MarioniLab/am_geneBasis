...
stat = readRDS(paste0(root.dir , "data/main_analysis/" , system , "/general_stat.Rds"))
...
### All genes we pick from selection
...
#     current.stat = get_celltypes_stat( sce , genes = current.genes , batch = "sample", nPC = 50 )
...
# saveRDS(stat_mapping_celltypes , file = paste0(root.dir , "data/main_analysis/" , system , "/stat_celltypes_all_genes.Rds"))
# 
# 
# # all markers
# stat_mapping_celltypes = bplapply(1:nrow(gene_selections.anno), function(i){
#   stat.per_selection = lapply(n_genes.grid , function(n_genes){
#     current.genes = gene_selections[[i]][1:n_genes]
#     current.genes = as.character(markers$gene[markers$gene %in% current.genes])
...
#   stat.per_selection = lapply(n_genes.grid , function(n_genes){
#     current.genes = gene_selections[[i]][1:n_genes]
#     current.genes = as.character(markers$gene[markers$gene %in% current.genes & markers$n.ct == 1])
#     if (length(current.genes) > 1) {
#       current.stat = get_celltypes_stat( sce , genes = current.genes , batch = "sample", nPC = 50 )
#       if (!is.null(current.stat)){
#         current.stat$n_genes = n_genes
...
# }, BPPARAM = mcparam)
# stat_mapping_celltypes = do.call(rbind , stat_mapping_celltypes)
# saveRDS(stat_mapping_celltypes , file = paste0(root.dir , "data/main_analysis/" , system , "/stat_celltypes_strong_markers.Rds"))
# 
...
saveRDS(stat_cell_score.nPC_50 , file = paste0(root.dir , "data/main_analysis/" , system , "/stat_cell_score_nPC_50.Rds"))
saveRDS(stat_cell_score.nPC_all , file = paste0(root.dir , "data/main_analysis/" , system , "/stat_cell_score_nPC_all.Rds"))
