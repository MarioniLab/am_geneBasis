...
gene_selections.files = setdiff(gene_selections.files , "old")
...
gene_selections.anno$descr.general = paste0(gene_selections.anno$assay , "_" , gene_selections.anno$type  )
gene_selections.anno$descr = paste0(gene_selections.anno$assay , "_" , gene_selections.anno$type , "_" , gene_selections.anno$run )


n_genes.grid = seq(25,200,25)

# just for atlas - rename the rownames
# assign gene-symbol names for the easier interpretation (for those which are dupped - ENSMUSG)
sce.big = readRDS(paste0( root.dir, "data/scRNA_datasets/" , system , "/sce_" , system , ".Rds"))
rowdata = as.data.frame(rowData(sce.big))
rm(sce.big)

tab = table(rowdata$SYMBOL)
tab = data.frame(SYMBOL = names(tab) , n = as.numeric(tab))
rowdata = merge(rowdata , tab)

rowdata$SYMBOL_unq = rowdata$SYMBOL
rowdata$SYMBOL_unq[rowdata$n > 1] = rowdata$ENSEMBL[rowdata$n > 1]

# assign as rownames
rownames.sce = rownames(sce)
rownames.sce = sapply(rownames.sce , function(gene) rowdata$SYMBOL_unq[rowdata$ENSEMBL == gene])
rownames(sce) = rownames.sce

```

# Get overall stat on all genes: corr, dists, mean and frac expressed
...
stat = lapply(assays , function(assay){
  ...
  dist.l1_stat = suppressWarnings( get_lp_norm_dist(sce , genes = rownames(sce), assay = assay , batch = "sample" , 
                          n.neigh = 5 , nPC = 50 , genes.predict = rownames(sce) , p = 1) )
  colnames(dist.l1_stat) = c("gene" , "dist.l1")
  dist.l2_stat = suppressWarnings( get_lp_norm_dist(sce , genes = rownames(sce), assay = assay , batch = "sample" , 
                          n.neigh = 5 , nPC = 50 , genes.predict = rownames(sce) , p = 2) )
  colnames(dist.l2_stat) = c("gene" , "dist.l2")
  dist.l3_stat = suppressWarnings( get_lp_norm_dist(sce , genes = rownames(sce), assay = assay , batch = "sample" , 
                          n.neigh = 5 , nPC = 50 , genes.predict = rownames(sce) , p = 3) )
  colnames(dist.l3_stat) = c("gene" , "dist.l3")
  dist.l4_stat = suppressWarnings( get_lp_norm_dist(sce , genes = rownames(sce), assay = assay , batch = "sample" , 
                          n.neigh = 5 , nPC = 50 , genes.predict = rownames(sce) , p = 4) )
  colnames(dist.l4_stat) = c("gene" , "dist.l4")
  ...
  current_stat = merge(corr_stat , dist.l1_stat)
  current_stat = merge(current_stat , dist.l2_stat)
  current_stat = merge(current_stat , dist.l3_stat)
  current_stat = merge(current_stat , dist.l4_stat)
  current_stat = merge(current_stat , mean_stat)
  current_stat = merge(current_stat , frac.expressed_stat)
  current_stat$assay.all = assay
  return(current_stat)
})
stat = do.call(rbind , stat)
saveRDS(stat , file = paste0(root.dir , "data/main_analysis/" , system , "/general_stat.Rds"))
...
#     current.genes = gene_selections[[i]][1:n_genes]
#     current.genes = unique( as.character(markers$gene[markers$gene %in% current.genes]) )
#     print(length(current.genes))
#     if (length(current.genes) > 1) {
#       current.stat = get_celltypes_stat( sce , genes = current.genes , batch = "sample", nPC =  2*length(unique(sce$celltype)) )
...
# saveRDS(stat_mapping_celltypes , file = paste0(root.dir , "data/main_analysis/" , system , "/stat_celltypes_all_markers.Rds"))
...
#                                   genes.compare = gene_selections[[i]][1:n_genes], batch = "sample", n.neigh = 5 , nPC.all = 50 , nPC.compare = length(current.genes)-2 ) )
#    if (!is.null(current.stat)){
#       current.stat$n_genes = n_genes
#       current.stat$id = i
...
# #stat_cell_score$nPC = factor(stat_cell_score$nPC)
# saveRDS(stat_cell_score.nPC_all , file = paste0(root.dir , "data/main_analysis/" , system , "/stat_cell_score.Rds"))
...
<!-- stat_gene_score$nPC = factor(stat_gene_score$nPC) -->

<!-- # only use stat for logcounts.denoised -->
<!-- stat = stat[stat$assay.all == "logcounts.denoised" , ] -->
<!-- colnames(stat)[which(colnames(stat) == "corr")] = "corr.all" -->
<!-- eps = 0.0001 -->
<!-- stat$corr.all[is.na(stat$corr.all)] = eps -->
<!-- stat$corr.all[stat$corr.all < eps] = eps -->

<!-- stat_gene_score$corr[is.na(stat_gene_score$corr)] = eps -->
<!-- stat_gene_score$corr[stat_gene_score$corr < eps] = eps -->
<!-- stat_gene_score = merge(stat_gene_score , stat[, c("gene" , "corr.all")]) -->
