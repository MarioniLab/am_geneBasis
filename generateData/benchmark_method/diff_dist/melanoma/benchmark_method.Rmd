  ...
  cell_score_stat = bplapply(c(1:length(genes)) , function(i){
    ...
    stat.per_n_genes = lapply(n_genes.grid , function(n_genes){
      current.stat = suppressWarnings( get_preservation_score(sce , neighs.all = neighs.all ,  genes.all = hvgs_10000, 
                                                 genes.compare = current.genes[1:n_genes], batch = NULL ,n.neigh = 5 , nPC.all = 50 , nPC.compare = NULL) )
...
saveRDS(cell_score_stat , file = paste0(root.dir , "data/benchmark_method/diff_dist/" , system , "/cell_score.Rds"))
...
stat_gene_score_all = suppressWarnings( get_corr_per_gene(sce, genes = hvgs_10000, batch = NULL , n.neigh = 5 , nPC = 50 , genes.predict = hvgs_10000 ) ) 
...
get_gene_score_stat = function(genes){
  gene_score_stat = bplapply(c(1:length(genes)) , function(i){
    ...
    stat.per_n_genes = lapply(n_genes.grid , function(n_genes){
      current.stat = suppressWarnings( get_corr_per_gene(sce , genes = current.genes[1:n_genes], batch = NULL , n.neigh = 5 , nPC = NULL , genes.predict = hvgs_10000 ) )
      current.stat$type = names(genes)[i]
      current.stat$n_genes = n_genes
      return(current.stat)
    })
...
saveRDS(gene_score_stat , file = paste0(root.dir , "data/benchmark_method/diff_dist/" , system , "/gene_score.Rds"))
